...
def prob_normalise(feature_map):
    feature_map = np.clip(feature_map, 0, None)
    total = np.sum(feature_map)
    if total > 0:
        return feature_map / total
    return feature_map
...
def compute_overall_saliency(frame, prev_frame=None):
    ...
    combined = prob_normalise(combined)
    return combined
...
for filename in sorted(os.listdir(STIMULI_DIR)):
    if filename.endswith(".mp4"):
        ...
        results.append({
            ...
            "affordance_saliency": avg_left,
            "control_saliency": avg_right
        })
        print(f"Done: {video_name} — affordance: {avg_left:.4f}, control: {avg_right:.4f}")
        
csv_path = os.path.join(OUTPUT_DIR, "salience_overall_distribution.csv")
with open(csv_path, 'w', newline='') as f:
    writer = csv.DictWriter(f, fieldnames=["video", "stimulus_label", "affordance_saliency", "control_saliency"])
    writer.writeheader()
    writer.writerows(results)
print(f"Saved CSV to {csv_path}")
            
affordance_vals = [r["affordance_saliency"] for r in results]
control_vals = [r["control_saliency"] for r in results]
...
ax.set_title("Distribution of visual salience", fontsize=20)
ax.tick_params(axis='both', labelsize=16)
plt.tight_layout()
plot_path = os.path.join(OUTPUT_DIR, "salience_overall_distribution.png")
